...
def generate_pdf_report(overall_score, details, job_description, resume_text):
    ...
    pdf.cell(0, 10, f"Matched Keywords: {', '.join(sorted(details['keyword']['matched'])) if details['keyword']['matched'] else 'None'}", ln=True)
    pdf.cell(0, 10, f"Missing Keywords: {', '.join(sorted(details['keyword']['missing'])) if details['keyword']['missing'] else 'None'}", ln=True)
    ...
    pdf.cell(0, 10, f"Present: {', '.join(details['headers']['present']) if details['headers']['present'] else 'None'}", ln=True)
    pdf.cell(0, 10, f"Missing: {', '.join(details['headers']['missing']) if details['headers']['missing'] else 'None'}", ln=True)
    ...
    weird_chars = ', '.join(set(details['format']['weird_chars'])) if details['format']['weird_chars'] else 'None'
