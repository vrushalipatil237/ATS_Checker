...
def sanitize_text(text):
    """Replace characters not supported by 'latin1' with '?'."""
...
def generate_pdf_report(overall_score, details, job_description, resume_text):
    ...
    pdf.cell(0, 10, f"Matched Keywords: {sanitize_text(', '.join(sorted(details['keyword']['matched'])) or 'None')}", ln=True)
    pdf.cell(0, 10, f"Missing Keywords: {sanitize_text(', '.join(sorted(details['keyword']['missing'])) or 'None')}", ln=True)
    ...
    pdf.cell(0, 10, f"Present: {sanitize_text(', '.join(details['headers']['present']) or 'None')}", ln=True)
    pdf.cell(0, 10, f"Missing: {sanitize_text(', '.join(details['headers']['missing']) or 'None')}", ln=True)
    ...
    weird_chars = sanitize_text(', '.join(set(details['format']['weird_chars'])) or 'None')
    ...
    pdf.multi_cell(0, 8, sanitize_text(jd_excerpt))
    ...
    pdf.multi_cell(0, 8, sanitize_text(resume_excerpt))
